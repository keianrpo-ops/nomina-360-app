...
// Reusing the same visual components from PayrollReceipt for consistency.
...
const SettlementReceipt: React.FC<SettlementReceiptProps> = ({ data }) => {
  const { employee } = data;
  const subtotalLiquidacion = data.cesantias + data.interesesCesantias + data.prima + data.vacaciones;
          ...
          <div style={{padding: '8px 12px', backgroundColor: 'white', borderBottomLeftRadius: '5px', borderBottomRightRadius: '5px'}}>
              {items.map((item, index) => (
                  <div key={index} style={{display: 'flex', justifyContent: 'space-between', padding: '6px 0', fontSize: '12px'}}>
                      ...
                      <span style={{color: item.value >= 0 ? '#2D3748' : '#E53E3E' }}>{formatCurrency(item.value)}</span>
                  </div>
              ))}
  ...
  return (
    <div style={{
      ...
      <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '8px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)', flexGrow: 1, display: 'flex', flexDirection: 'column' }}>

        {/* HEADER */}
        <header>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', paddingBottom: '15px' }}>
              <div>
                <h1 style={{ margin: 0, fontSize: '28px', color: '#2C5282', fontWeight: 'bold' }}>Recibo de Liquidación</h1>
                <p style={{ margin: '5px 0 0', color: '#718096' }}>Terminación de Contrato</p>
              </div>
        ...
        <section style={{ marginTop: '20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px 24px' }}>
              ...
              <InfoRow label="Fecha de Retiro" value={data.fechaRetiro} />
          </div>
        </section>
        
        {/* SUMMARY BOXES */}
        <section style={{ marginTop: '25px', display: 'flex', gap: '20px' }}>
          <SummaryBox title="Días de Antigüedad" value={`${data.diasAntiguedad} DÍAS`} color="#EBF8FF" />
          <SummaryBox title="Salario Base (Prestaciones)" value={formatCurrency(data.salarioBasePromedio)} color="#E6FFFA" />
        </section>
        ...
        <section style={{ marginTop: '25px', display: 'flex', gap: '20px' }}>
            <DetailTable 
                ...
                items={[
                    ...
                    { label: 'Intereses sobre Cesantías', value: data.interesesCesantias },
            ...
            <DetailTable 
                ...
                titleColor="#2A4365"
                items={[
                    { label: 'Otros Conceptos (Bonos, etc.)', value: data.otrosConceptos },
                    { label: 'Deducciones (Préstamos, etc.)', value: -data.deducciones },
                ]}
        ...
        <section style={{ marginTop: '30px', backgroundColor: '#2C5282', color: 'white', padding: '20px', borderRadius: '8px', textAlign: 'center' }}>
          <p style={{ margin: 0, fontSize: '14px', textTransform: 'uppercase', letterSpacing: '1px' }}>Total Liquidación a Pagar</p>
          <p style={{ margin: '5px 0 0', fontSize: '32px', fontWeight: 'bold' }}>{formatCurrency(data.totalLiquidacion)}</p>
        </section>
        ...
        <footer style={{ marginTop: '20px' }}>
            <p style={{ textAlign: 'center', fontSize: '10px', color: '#A0AEC0', margin: '0 0 25px 0' }}>
                Documento generado automáticamente. Los cálculos son referenciales y deben ser validados según la normativa vigente y políticas de la empresa.
            </p>
             <div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'flex-end', paddingTop: '20px', borderTop: '1px solid #E2E8F0' }}>
                <div style={{ textAlign: 'center', fontSize: '12px', color: '#4A5568' }}>
                    <div style={{ borderBottom: '1px solid #718096', height: '40px', width: '250px', marginBottom: '8px' }}></div>
                    <p style={{margin: 0}}>Firma Empleado</p>
                    <p style={{margin: '2px 0 0', fontSize: '10px'}}>C.C. {employee.Cedula}</p>
                </div>
                <div style={{ textAlign: 'center', fontSize: '12px', color: '#4A5568' }}>
                    <div style={{ borderBottom: '1px solid #718096', height: '40px', width: '250px', marginBottom: '8px' }}></div>
                    <p style={{margin: 0}}>Firma Empleador</p>
                    <p style={{margin: '2px 0 0', fontSize: '10px'}}>Empresa S.A.S</p>
                </div>
            </div>
        </footer>
